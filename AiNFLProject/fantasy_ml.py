...
def build_defense_vs_position_features(stats: pl.DataFrame, fantasy_col: str) -> pl.DataFrame:
    ...
    raw = (
        ...
        .agg(pl.col(fantasy_col).sum().alias("def_fp_allowed"))
        .sort([opp_col, "week"])
    )

    # Compute rolling stats in pandas for broad Polars compatibility
    raw_pd = raw.to_pandas()
    frames = []
    for def_team, grp in raw_pd.groupby(opp_col):
        grp = grp.sort_values("week").copy()
        prev = grp["def_fp_allowed"].shift(1)
        grp["def_fp_prev1"] = prev
        grp["def_fp_roll3"] = prev.rolling(window=3).mean()
        grp["def_fp_roll5"] = prev.rolling(window=5).mean()
        grp["def_fp_season_avg"] = prev.expanding().mean()
        frames.append(grp)

    def_feats = pl.from_pandas(pd.concat(frames, ignore_index=True))
...
def build_upcoming_week_features(max_week: int, season: int, position: str, scoring: str) -> pd.DataFrame:
    ...
    feats_df = feats_df.dropna(subset=["def_fp_prev1", "def_fp_roll3", "def_fp_roll5", "def_fp_season_avg"])
